...
const codeSnippets: CodeSnippet[] = [
  ...
  { lang: 'JavaScript', code: `function slidingWindow(s) {
  let map = {};
  let left = 0, maxLen = 0;
  for (let right = 0; right < s.length; right++) {
    map[s[right]] = (map[s[right]] || 0) + 1;
    // 🐛 Missing: shrink window when duplicate found
    ...
    for (int i = 1; i <= amount; i++) {
        for (int c : coins) {
            if (c <= i && dp[i-c] != INT_MAX)
                dp[i] = dp[i-c] + 1; // 🐛 Bug: should be min()
        }
    }
    return dp[amount];
}` },
...
const AuthPage = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => setSnippetIndex(i => (i + 1) % 3), 4000);
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="ml-2 text-xs font-code text-text-secondary">{codeSnippets[snippetIndex].lang}</span>
              </div>
              <pre className="font-code text-xs text-text-primary leading-relaxed whitespace-pre-wrap">{codeSnippets[snippetIndex].code}</pre>
            </motion.div>
          </AnimatePresence>
          <p className="mt-8 font-display text-2xl text-text-primary">
            Your AI mentor that <span className="text-accent-teal">never forgets</span>
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md bg-bg-card rounded-lg border border-border p-8">
          ...
          <div className="relative flex bg-bg-code rounded-md p-1 mb-6">
            ...
            <motion.div
              ...
              style={{ width: '50%', left: mode === 'signin' ? '0%' : '50%' }}
              transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            />
          </div>

          {/* GitHub */}
          <button className="w-full flex items-center justify-center gap-2 py-2.5 rounded-md border border-border text-text-primary font-body text-sm hover:bg-bg-code transition-colors mb-4">
