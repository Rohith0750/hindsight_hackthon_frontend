...
import { Trophy, Flame, TrendingUp, Lightbulb, Star, ChevronDown } from 'lucide-react';
...
const ProgressPage = () => {
  const { user } = useAuth();
  const { data: insights, isLoading } = useMistakePatterns(user?.id || '');
  const { data: proficiencyData } = useUserProficiency();
  const { data: statsData } = useUserStats();

  const proficiency = proficiencyData as unknown as Proficiency[] | null;
  const stats = statsData as unknown as Stats | null;

  const [expandedPattern, setExpandedPattern] = useState<string | null>(null);

  const radarData =
    proficiency?.map((l: Proficiency) => ({
      subject: l.language,
      value: l.solved,
      fullMark: 10,
    })) || [];
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          { icon: Lightbulb, label: 'Hints Used', value: 0, color: 'text-yellow-400' },
          { icon: Star, label: 'Hints Earned', value: 0, color: 'text-purple-400' },
        ].map(s => (
        ...
        <motion.div variants={item} className="space-y-3">
          ...
          {isLoading ? (
            ...
            insights?.patterns.map(p => (
              <div key={p.id} className="bg-bg-card rounded-lg border border-border overflow-hidden">
                <button
        ...
        <motion.div variants={item} className="space-y-3">
          ...
          <div className="grid gap-3">
            {insights?.insights.map(i => (
              <div key={i.id} className="bg-bg-code rounded-lg p-4 border border-border">
                ...
                <Link href={i.actionLink} className="text-xs font-display text-accent-teal hover:underline mt-2 inline-block">{i.actionLabel} →</Link>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <div className="relative pl-6 space-y-4">
          ...
          {SUBMISSIONS.slice(0, 8).map(s => (
            <div key={s.id} className="relative flex items-start gap-3">
              <div className={`absolute left-[-16px] w-3 h-3 rounded-full border-2 ${s.verdict === 'Accepted' ? 'bg-accent-teal border-accent-teal' : 'bg-fire-accent border-fire-accent'
                }`} />
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className="text-xs font-body text-text-secondary">{new Date(s.timestamp).toLocaleDateString()}</span>
                </div>
              </div>
            </div>
          ))}
