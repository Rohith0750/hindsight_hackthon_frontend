        ...
        <div className="flex gap-2 flex-wrap">
          {difficulties.map((d) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-display border transition-colors ${difficulty === d
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-display border flex items-center gap-1 transition-colors ${aiPicks
                ? "bg-fire-accent/20 border-fire-accent text-fire-accent"
                : "border-border text-text-secondary hover:text-text-primary"
              }`}
          >
      ...
      {aiPicks && (
        <div className="bg-bg-code rounded-lg p-3 border-l-4 border-fire-accent">
          <p className="text-sm font-body text-text-secondary">
            🎯 Showing AI-selected problems based on your mistake patterns
          </p>
        </div>
      )}
          ...
          {filtered?.map((p, index) => (
            <motion.div key={p.id} variants={item}>
              <Link
                ...
                className="grid sm:grid-cols-[50px_1fr_100px_180px_100px_100px] items-center gap-3 px-4 py-3 border-b border-border/50 hover:bg-bg-code/50 transition-all"
              >
                {/* Index */}
                ...
                {/* Difficulty */}
                <span
                  className={`text-xs font-semibold px-2 py-0.5 rounded w-fit ${p.difficulty === "Easy"
                      ? "bg-accent-teal/20 text-accent-teal"
                      : p.difficulty === "Medium"
                        ? "bg-fire-accent/20 text-fire-accent"
                ...
                <div className="flex gap-1 flex-wrap">
                  {p.tags?.slice(0, 2).map((t: string, i: number) => (
                    <span
                      key={i}
                      className="text-[10px] px-2 py-0.5 rounded bg-bg-code text-text-secondary"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <span className="text-xs font-medium">
                  {p.aiPriority === "High" ? (
                    <span className="text-fire-accent">🎯 High</span>
                  ) : p.aiPriority === "Medium" ? (
                    <span className="text-accent-teal">⭐ Med</span>
                  ) : (
